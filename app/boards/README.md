...
When a target only needs abstract OpenDeck UART/I2C labels for bootloader DTS
parsing, those alias lines are generated automatically from the target `firmware.overlay`.
Only targets with real bootloader-specific DTS changes need dedicated board-layer
`bootloader.overlay` files under `app/boards/zephyr/<board>/`.
