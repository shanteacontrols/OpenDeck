...
3. `app/boards/zephyr/<board>/partitions.overlay`
4. `app/boards/zephyr/<board>/firmware.overlay`
5. `app/boards/opendeck/<target>/common.overlay`, if present
6. `app/boards/opendeck/<target>/firmware.overlay`
7. generated firmware USB MIDI label overlay
...
2. `app/boards/zephyr/<board>/common.overlay`, if present
3. `app/boards/zephyr/<board>/partitions.overlay`
4. `app/boards/zephyr/<board>/bootloader.overlay`
5. `app/bootloader/firmware_loader.overlay`
6. `app/boards/opendeck/<target>/common.overlay`, if present
7. `app/boards/opendeck/<target>/bootloader.overlay`

MCUboot image overlay order:

1. upstream Zephyr board DTS
2. `app/sysbuild/mcuboot.overlay`
3. `app/boards/zephyr/<board>/common.overlay`, if present
4. `app/boards/zephyr/<board>/partitions.overlay`
5. `app/boards/opendeck/<target>/common.overlay`, if present
6. `app/boards/opendeck/<target>/bootloader.overlay`
7. `app/sysbuild/mcuboot.overlay`
...
3. Add the normal board-layer files:
   - `common.overlay`, only if firmware and bootloader images need the same low-level DTS changes
   - `partitions.overlay`
   - `firmware.overlay`
   - `firmware.conf`
   - `bootloader.conf`
...
- `app/boards/zephyr/<board>/firmware.overlay` should own reusable low-level peripheral setup and common board setup
