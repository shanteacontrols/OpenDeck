...
void Buttons::updateSingle(size_t index, bool forceRefresh)
{
    ...
    if (!forceRefresh)
    {
        ...
        bool filteredState = currentState.value();

        if (!_filter.isFiltered(index, filteredState))
        {
            return;
        }

        processButton(index, filteredState, descriptor);
    }
    else
    {
        fillDescriptor(index, descriptor);
        ...
        {
            sendMessage(index, cachedState(index), descriptor);
        }
    }
}
...
void Buttons::sendMessage(size_t index, bool state, Descriptor& descriptor)
{
    ...
    if (state)
    {
        ...
        case messageType_t::MULTI_VAL_INC_RESET_NOTE:
        {
            ...
            if (newValue != _incDecValue[index])
            {
                if (!newValue)
                {
                    descriptor.signal.message = protocol::midi::messageType_t::NOTE_OFF;
                }
                else
                {
                    descriptor.signal.message = protocol::midi::messageType_t::NOTE_ON;
                }

                _incDecValue[index]     = newValue;
                descriptor.signal.value = newValue;
            }
            else
            {
                send = false;
        ...
        {
            MidiProgram.incrementOffset(descriptor.signal.value);
        }
        break;

        case messageType_t::PROGRAM_CHANGE_OFFSET_DEC:
        {
            MidiProgram.decrementOffset(descriptor.signal.value);
        }
        break;
        ...
        case messageType_t::PRESET_CHANGE:
        {
            ...
            event.systemMessage           = messaging::systemMessage_t::PRESET_CHANGE_DIRECT_REQ;
    ...
    else
    {
        ...
        case messageType_t::MMC_RECORD:
        {
            descriptor.signal.message = protocol::midi::messageType_t::MMC_RECORD_STOP;
        }
        break;

        case messageType_t::MMC_PLAY_STOP:
        {
            descriptor.signal.message = protocol::midi::messageType_t::MMC_STOP;
        }
        break;
...
void Buttons::setState(size_t index, bool state)
{
    ...
    misc::bit_write(_buttonPressed[arrayIndex], bit, state);
}
...
bool Buttons::cachedState(size_t index)
{
    uint8_t arrayIndex = index / 8;
    uint8_t bit        = index - 8 * arrayIndex;

    return misc::bit_read(_buttonPressed[arrayIndex], bit);
}
...
void Buttons::setLatchingState(size_t index, bool state)
{
    ...
    misc::bit_write(_lastLatchingState[arrayIndex], bit, state);
}
...
bool Buttons::latchingState(size_t index)
{
    ...
    return misc::bit_read(_lastLatchingState[arrayIndex], bit);
}
...
void Buttons::fillDescriptor(size_t index, Descriptor& descriptor)
{
    ...
    descriptor.signal.channel        = _database.read(database::Config::Section::button_t::CHANNEL, index);
    descriptor.signal.index          = _database.read(database::Config::Section::button_t::MIDI_ID, index);
    descriptor.signal.value          = _database.read(database::Config::Section::button_t::VALUE, index);
    ...
    descriptor.signal.message = INTERNAL_MSG_TO_MIDI_TYPE[static_cast<uint8_t>(descriptor.messageType)];
}
