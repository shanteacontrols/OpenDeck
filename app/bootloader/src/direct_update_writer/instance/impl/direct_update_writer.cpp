...
direct_update_writer::DirectUpdateWriter::DirectUpdateWriter(Hwa& hwa)
    : _hwa(hwa)
    , _writer(*this)
{}

void direct_update_writer::DirectUpdateWriter::reset()
{
    reset_state(true);
}

bool direct_update_writer::DirectUpdateWriter::begin([[maybe_unused]] const dfu_stream::Header& header,
                                                     const uint32_t                             size)
{
    reset();
...
bool direct_update_writer::DirectUpdateWriter::write(std::span<const uint8_t> data)
{
    if (data.empty())
    {
        return true;
...
bool direct_update_writer::DirectUpdateWriter::finish()
{
    if (_failed || !_active)
    {
...
void direct_update_writer::DirectUpdateWriter::abort()
{
    reset();
}

void direct_update_writer::DirectUpdateWriter::reset_state(const bool clear_failure)
{
    _current_sector        = 0;
...
bool direct_update_writer::DirectUpdateWriter::payload_fits(const uint32_t size)
{
    return size <= _hwa.size();
}

bool direct_update_writer::DirectUpdateWriter::prepare_write_block_size()
{
...
bool direct_update_writer::DirectUpdateWriter::write_payload_byte(const uint8_t data)
{
    const auto sector_size = _hwa.sector_size(_current_sector);
...
bool direct_update_writer::DirectUpdateWriter::flush_write_block()
{
    if (!_writer.flush())
    {
...
bool direct_update_writer::DirectUpdateWriter::write_block(const uint32_t offset, std::span<const uint8_t> data)
{
...
bool direct_update_writer::DirectUpdateWriter::finish_current_sector()
{
...
bool direct_update_writer::DirectUpdateWriter::fail(std::string_view status)
{
    this->status(status);
    _failed = true;
    reset_state(false);
...
void direct_update_writer::DirectUpdateWriter::status(std::string_view message)
{
