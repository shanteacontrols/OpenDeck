...
{
    DatabaseInfo info    = {};
    info.type            = static_cast<Type>(_database.read(database::Config::Section::Button::Type, index));
    info.message_type    = static_cast<MessageType>(_database.read(database::Config::Section::Button::MessageType, index));
    info.component_index = index;
    info.channel         = _database.read(database::Config::Section::Button::Channel, index);
    info.midi_index      = _database.read(database::Config::Section::Button::MidiId, index);
    info.midi_value      = _database.read(database::Config::Section::Button::Value, index);
                                                   ...
                                                   bool                refresh)
{
    ...
    if (midi_send)
    {
        ...
        if (send)
        {
            signal.source = signaling::IoEventSource::Button;
            result.midi   = signal;
        }
    }
...
{
    result.osc->source          = signaling::IoEventSource::Button;
