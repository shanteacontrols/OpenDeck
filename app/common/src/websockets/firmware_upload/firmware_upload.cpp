...
using namespace opendeck::websockets;

FirmwareUpload::FirmwareUpload(dfu_stream::Sink& sink)
    : _sink(sink)
    , _dfu_stream(sink)
{}
...
FirmwareUploadCommandResult FirmwareUpload::handle_chunk(std::span<const uint8_t> payload)
{
    ...
    return result(FirmwareUploadCommand::Chunk,
                  status == dfu_stream::StreamStatus::Invalid ? FirmwareUploadStatus::Failed
