...
core_yaml_file=$(make --no-print-directory -C "$ZEPHYR_WS"/modules/libcore MCU="$mcu" print-MCU_YML_FILE)
...
if [[ ! -d $gen_dir ]]
then
    if ! make --no-print-directory -C "$ZEPHYR_WS"/modules/libcore MCU="$mcu" MCU_GEN_DIR="$gen_dir" MCU_GEN_USB=1 MCU_EXT_CLOCK_MHZ="$extClockMhz"
    then
        exit 1
    fi
