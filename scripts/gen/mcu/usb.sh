...
then
    ...
    then
        ...
        {
            printf "%s\n" "DEFINES += USB_ENDPOINT_SIZE_CONTROL=$usb_endpoint_size_control"
            printf "%s\n" "DEFINES += USB_ENDPOINT_SIZE_MIDI_IN_OUT=$usb_endpoint_size_midi_in_out"
            printf "%s\n" "DEFINES += USB_ENDPOINT_SIZE_CDC_IN_OUT=$usb_endpoint_size_cdc_in_out"
            printf "%s\n" "DEFINES += USB_ENDPOINT_SIZE_CDC_NOTIFICATION=$usb_endpoint_size_cdc_notification"
        } >> "$out_makefile"
    fi
