...
sudo avrdude -p "$mcu" -P usb -c avrispmkII -C /etc/avrdude.conf -e -V -u -U lock:w:"$fuse_unlock":m -U efuse:w:"$fuse_ext":m -U hfuse:w:"$fuse_high":m -U lfuse:w:"$fuse_low":m
sudo avrdude -p "$mcu" -P usb -c avrispmkII -C /etc/avrdude.conf -U flash:w:"$PWD"/merged.hex
sudo avrdude -p "$mcu" -P usb -c avrispmkII -C /etc/avrdude.conf -V -u -U lock:w:"$fuse_lock":m