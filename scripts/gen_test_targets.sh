...
do
...

    {
        ...
        printf '\t%s\n' '@mkdir -p $(@D)'
        printf '\t%s\n' '@echo Building: $<'
        printf '\t%s\n' '@ccache $(CPP_COMPILER) $(COMMON_FLAGS) $(CPP_FLAGS) $(INCLUDE_FILES_COMMON) $(INCLUDE_DIRS_COMMON) $(addprefix -I$(FW_ROOT_DIR)/,$(INCLUDE_DIRS_'${test}')) $(addprefix -D,$(DEFINES)) $(addprefix -D,$(DEFINES_'${test}')) -MD -MP -MF "$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -MT"$(@:%.o=%.o)" -c "$<" -o "$@"'

        printf '%s\n' '$(BUILD_DIR)/$(TEST_DIR_'${test}')/%.c.o: %.c'
        printf '\t%s\n' '@mkdir -p $(@D)'
        printf '\t%s\n' '@echo Building: $<'
        printf '\t%s\n' '@ccache $(C_COMPILER) $(COMMON_FLAGS) $(C_FLAGS) $(INCLUDE_FILES_COMMON) $(INCLUDE_DIRS_COMMON) $(addprefix -I$(FW_ROOT_DIR)/,$(INCLUDE_DIRS_'${test}')) $(addprefix -D,$(DEFINES)) $(addprefix -D,$(DEFINES_'${test}')) -MD -MP -MF "$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -MT"$(@:%.o=%.o)" -c "$<" -o "$@"'
