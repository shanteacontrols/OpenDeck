...
namespace
{
    ...
    const std::string usb_power_off_cmd        = "uhubctl -a off -l 2-1.4 > /dev/null";
    const std::string usb_power_on_cmd         = "uhubctl -a on -l 2-1.4 > /dev/null";
    ...
    for (int preset = 0; preset < database.getSupportedPresets(); preset += (database.getSupportedPresets() - 1))
    {
        ...
        for (size_t group = 0; group < IO::Buttons::Collection::groups(); group++)
        {
            for (size_t i = 0; i < IO::Buttons::Collection::size(group); i += PARAM_SKIP)
                TEST_ASSERT_EQUAL_UINT32(i, MIDIHelper::readFromBoard(System::Section::button_t::midiID, i + IO::Buttons::Collection::startIndex(group)));
        }
        ...
        // midi id section
        for (size_t i = 0; i < IO::Encoders::Collection::size(); i += PARAM_SKIP)
            TEST_ASSERT_EQUAL_UINT32(i, MIDIHelper::readFromBoard(System::Section::encoder_t::midiID, i));
        ...
        for (size_t group = 0; group < IO::LEDs::Collection::groups(); group++)
        {
            for (size_t i = 0; i < IO::LEDs::Collection::size(group); i += PARAM_SKIP)
                TEST_ASSERT_EQUAL_UINT32(i, MIDIHelper::readFromBoard(System::Section::leds_t::activationID, i + IO::LEDs::Collection::startIndex(group)));
        }

        // rgb enable section
        // all values should be set to 0
        for (size_t i = 0; i < MAX_NUMBER_OF_IO::LEDs::Collection::size() / 3 + (IO::Touchscreen::Collection::size() / 3); i += PARAM_SKIP)
...
TEST_CASE(MIDIData)
{
    ...
    TEST_ASSERT(stoi(response) >= (IO::Buttons::Collection::size(IO::Buttons::GROUP_DIGITAL_INPUTS) / 2));
