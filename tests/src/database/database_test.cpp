...
namespace
{
    ...
    class CountingHandlers : public database::Handlers
    {
        ...
        void factoryResetStart() override
        {
            factoryResetStartCount++;
        }

        void factoryResetDone() override
        {
            factoryResetDoneCount++;
        }
        ...
        size_t factoryResetStartCount = 0;
        size_t factoryResetDoneCount  = 0;
    ...
    class DatabaseTest : public ::testing::Test
    {
        ...
        void SetUp() override
        {
            ...
            ASSERT_EQ(0, _database.instance().currentPreset());
        }
...
#define DB_COMMON_READ_VERIFY(expected, value)                                                      \
    do                                                                                              \
    {                                                                                               \
        ASSERT_TRUE(_database.instance().read(database::Config::Section::common_t::COMMON_SETTINGS, \
                                              value,                                                \
                                              _dbReadRetVal));                                      \
        ASSERT_EQ(expected, _dbReadRetVal);                                                         \
    } while (0)
...
{
    const auto expectedSignature = static_cast<uint16_t>(_database._layout.presetUid() ^
                                                         static_cast<uint16_t>(PROJECT_TARGET_UID) ^
                                                         static_cast<uint16_t>(_database.instance().getSupportedPresets()));
    ...
    for (int preset = 0; preset < _database.instance().getSupportedPresets(); preset++)
    {
        ...
        // common block
        //----------------------------------
        DB_COMMON_READ_VERIFY(preset, database::Config::commonSetting_t::ACTIVE_PRESET);
        DB_COMMON_READ_VERIFY(0, database::Config::commonSetting_t::PRESET_PRESERVE);

        for (int i = static_cast<int>(database::Config::commonSetting_t::CUSTOM_COMMON_SETTING_START);
             i < static_cast<int>(database::Config::commonSetting_t::CUSTOM_COMMON_SETTING_END);
             i++)
        {
            DB_COMMON_READ_VERIFY(0, i);
        }

        DB_COMMON_READ_VERIFY(expectedSignature, database::Config::commonSetting_t::UID);
...
TEST(DatabaseRegressionTest, InitWithMissingFactorySnapshotFallsBackToFactoryReset)
{
    ...
    EXPECT_EQ(1U, handlers.factoryResetStartCount);
    EXPECT_EQ(1U, handlers.factoryResetDoneCount);
    EXPECT_EQ(1U, handlers.initializedCount);

    EXPECT_TRUE(builder.instance().isInitialized());
    EXPECT_EQ(0, builder.instance().currentPreset());

    ASSERT_TRUE(builder.instance().read(database::Config::Section::common_t::COMMON_SETTINGS,
                                        database::Config::commonSetting_t::UID,
                                        readValue));
    EXPECT_NE(0U, readValue);
}
...
TEST_F(DatabaseTest, Presets)
{
    ...
    for (activePreset = 0; activePreset < _database.instance().getSupportedPresets(); activePreset++)
    {
        ...
        ASSERT_EQ(activePreset, _database.instance().currentPreset());
    }
