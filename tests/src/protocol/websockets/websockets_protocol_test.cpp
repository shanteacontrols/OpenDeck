...
namespace
{
    ...
    class WebSocketsProtocolTest : public ::testing::Test
    {
        protected:
...
TEST_F(WebSocketsProtocolTest, StartsAndStopsServerThroughHwa)
{
    ASSERT_TRUE(websockets.init());
    ASSERT_TRUE(websockets.deinit());
...
TEST_F(WebSocketsProtocolTest, PublishesBinarySysExFramesAsConfigRequests)
{
    ConfigRequestCollector collector;
    ...
    EXPECT_EQ(collector.transport(0), signaling::ConfigTransport::WebSockets);
...
TEST_F(WebSocketsProtocolTest, IgnoresNonBinaryFrames)
{
    ConfigRequestCollector collector;
...
TEST_F(WebSocketsProtocolTest, ReplacesActiveClient)
{
    ASSERT_TRUE(websockets.init());
    ASSERT_EQ(websockets.accept_client(CLIENT_SOCKET), 0);
    ASSERT_EQ(websockets.accept_client(CLIENT_SOCKET + 1), 0);
...
TEST_F(WebSocketsProtocolTest, SendsConfigResponsesAsSysExBytes)
{
    ASSERT_TRUE(websockets.init());
    ASSERT_EQ(websockets.accept_client(CLIENT_SOCKET), 0);
...
TEST_F(WebSocketsProtocolTest, FirmwareUploadRequestsBootloaderRebootThroughSystem)
{
...
TEST_F(WebSocketsProtocolTest, FirmwareUploadBeginClosesActiveConfigSession)
{
...
TEST_F(WebSocketsProtocolTest, DropsConfigResponsesFromStaleClientSession)
{
    ASSERT_TRUE(websockets.init());
    ASSERT_EQ(websockets.accept_client(CLIENT_SOCKET), 0);
...
TEST_F(WebSocketsProtocolTest, StaleConfigResponseDoesNotClearActivePartialResponse)
{
    ASSERT_TRUE(websockets.init());
    ASSERT_EQ(websockets.accept_client(CLIENT_SOCKET), 0);
