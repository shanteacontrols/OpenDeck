...
namespace
{
    ...
    {
        public:
        bool begin(const opendeck::common::dfu::dfu_stream::Header& header, const uint32_t size) override
        {
            begin_called++;
    ...
    opendeck::common::dfu::dfu_stream::StreamStatus feed(opendeck::common::dfu::dfu_stream::DfuStream& parser,
                                                         const std::vector<uint8_t>&                   stream)
    {
        auto status = opendeck::common::dfu::dfu_stream::StreamStatus::Incomplete;
...
TEST(DfuStream, AcceptsValidStream)
{
    ...
    SinkTest                                     sink;
    opendeck::common::dfu::dfu_stream::DfuStream parser(sink);

    const auto status = feed(parser, opendeck::tests::dfu_stream::make_stream(payload));

    ASSERT_EQ(status, opendeck::common::dfu::dfu_stream::StreamStatus::Complete);
...
TEST(DfuStream, AcceptsStreamInChunks)
{
    ...
    ASSERT_EQ(first, opendeck::common::dfu::dfu_stream::StreamStatus::Incomplete);
    ASSERT_EQ(rest, opendeck::common::dfu::dfu_stream::StreamStatus::Complete);
    ASSERT_EQ(sink.payload, payload);
}
...
TEST(DfuStream, RejectsWrongStartMagic)
{
    ...
    ASSERT_EQ(parser.feed(stream[0]), opendeck::common::dfu::dfu_stream::StreamStatus::Invalid);
...
TEST(DfuStream, RejectsUnsupportedFormat)
{
    ...
    const auto status = feed(parser, opendeck::tests::dfu_stream::make_stream({ 0x01 }, OPENDECK_TARGET_UID, opendeck::common::dfu::dfu_stream::FORMAT_VERSION + 1));

    ASSERT_EQ(status, opendeck::common::dfu::dfu_stream::StreamStatus::Invalid);
    ASSERT_EQ(sink.begin_called, 0);
...
TEST(DfuStream, RejectsWrongEndMagicAndAbortsActiveSink)
{
    ...
    const auto status = feed(parser, opendeck::tests::dfu_stream::make_stream({ 0x01, 0x02 }, OPENDECK_TARGET_UID, opendeck::common::dfu::dfu_stream::FORMAT_VERSION, 0));
