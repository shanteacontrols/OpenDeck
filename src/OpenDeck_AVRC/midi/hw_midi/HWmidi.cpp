...
bool MIDI_Class::parse(byte inChannel)  {
        ...
        if (mPendingMessageIndex == 0)  { //HwMIDIstart a new pending message
...
void MIDI_Class::setHandleHwMIDIpitchBend(void (*fptr)(byte channel, int bend))   {

    mHwMIDIpitchBendCallback = fptr;

}

void MIDI_Class::setHandleSystemExclusive(void (*fptr)(byte * array, byte size)) {
