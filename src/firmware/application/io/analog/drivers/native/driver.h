...
namespace io::analog
{
    class HwaHw
    {
        ...
        bool init()
        {
            ...
            {
                auto pin = target::pins::ADC_PIN(i);
            ...
            for (uint8_t i = 0; i < 3; i++)
            {
                ...
                core::mcu::adc::read(target::pins::ADC_PIN(0));
            }

            core::mcu::adc::setActivePin(target::pins::ADC_PIN(0));
            core::mcu::adc::enableIt(ISR_PRIORITY, [this](uint16_t value)
                                     {
                                         isr(value);
                                     });
        ...
        private:
        static constexpr inline uint8_t ISR_PRIORITY                        = 5;
        static constexpr size_t         ANALOG_IN_BUFFER_SIZE               = PROJECT_TARGET_MAX_NR_OF_ANALOG_INPUTS;
        ...
        void isr(uint16_t adcValue)
        {
            if (adcValue <= CORE_MCU_ADC_MAX_VALUE)
            {
                ...
                if (++sampleCounter == (PROJECT_MCU_ADC_SAMPLES + 1))
                {
                    ...
                    core::mcu::adc::setActivePin(target::pins::ADC_PIN(analogIndex));
                }
            }
