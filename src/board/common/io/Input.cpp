...
namespace
{
    ...
    inline void storeDigitalIn()
    {
        ...
        {
            _portBuffer[portIndex].insert(CORE_MCU_IO_READ_IN_PORT(Board::detail::map::digitalInPort(portIndex)));
        }
    }
        ...
        while (_portBuffer[portIndex].remove(portValue))
        {
            for (int i = 0; i < NR_OF_DIGITAL_INPUTS; i++)
            {
                if (Board::detail::map::buttonPortIndex(i) == portIndex)
                {
                    ...
                    _digitalInBuffer[i].readings |= !core::util::BIT_READ(portValue, Board::detail::map::buttonPinIndex(i));
