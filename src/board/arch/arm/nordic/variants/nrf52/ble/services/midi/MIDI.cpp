...
namespace
{
    ...
    core::RingBuffer<uint8_t, MIDI_BLE_MAX_PACKET_SIZE> _rxBuffer;
    ...
    uint32_t dataIOcharAdd(midiService_t& midiService)
    {
        ...
        BOARD_ERROR_CHECK(sd_ble_uuid_vs_add(&baseUuid, &midiService.uuidType), NRF_SUCCESS);
        ...
        attrCharValue.max_len   = MIDI_BLE_MAX_PACKET_SIZE;
...
namespace Board::BLE::MIDI
{
    ...
    {
        if (maxSize > MIDI_BLE_MAX_PACKET_SIZE)
        {
            return false;
        }
